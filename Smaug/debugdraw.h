...
public:
	...
	void Line(vertex_t* start, vertex_t* end, glm::vec3 color = { 1.0f,0.0f,1.0f }, float width = 1.0f, float decay = 5.0f) { Line(*start->vert + start->edge->face->meshPart->mesh->origin, *end->vert + end->edge->face->meshPart->mesh->origin, color, width, decay); }
	...
	void LineDelta(vertex_t* start, glm::vec3 delta, glm::vec3 color = { 1.0f,0.0f,1.0f }, float width = 1.0f, float decay = 5.0f) { LineDelta(*start->vert + start->edge->face->meshPart->mesh->origin, delta, color, width, decay); }
